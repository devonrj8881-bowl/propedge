...
function extractPlayersFromCSV(csvPath) {
  try {
    ...
    if (playerIdx < 0 || propIdx < 0) {
      Logger.error(`CSV missing Player or Prop columns`);
      return [];
    }
    ...
    Logger.log(`Extracted ${players.size} unique players from CSV`);
...
function extractStatsFromCSV(csvPath, league) {
  ...
  try {
    ...
    const l5Idx = headers.findIndex(h => h === 'l5 avg');
    const l10Idx = headers.findIndex(h => h === 'l10 avg');
...
async function enrichCSV(csvPath, league, date) {
  ...
  Logger.success(`Enriched ${enrichedData.length} props and wrote to: ${outputPath}`);

  return {
    outputPath,
    enrichedCount: enrichedData.length,
    statsFound: statsUsed,
    coverage: ((statsUsed / enrichedData.length) * 100).toFixed(1) + '%'
  };
}
