...
function addEnrichmentColumns(headers, data, league) {
  ...
  const enrichedData = data.map(row => {
    ...
    // Calculate confidence from L10/L5 consistency (0-100%)
    // High consistency + high values = high confidence
    let confidence = 50; // default
    if (l10 > 0) {
      const consistency = Math.max(0, 1 - Math.abs(l10 - l5) / (l10 + 0.1)); // 0-1
      confidence = Math.round((l10 / 10 + consistency * 50) * 0.8 + 10); // 10-100
...
async function processLeague(page, sheets, league) {
  ...
  if (enrichedData.length > 0) {
    ...
    await writeToSheet(sheets, league, enrichedHeaders, enrichedData);
  } else {
...
async function main() {
  ...
  try {
    ...
    await page.goto('https://propfinder.app/login', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await sleep(2000);
