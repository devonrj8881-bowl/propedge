...
**Always deploy via Netlify CLI — do not use GitHub PRs or git push as a deployment method.**

Steps after changes are ready:
1. Copy updated `index.html` to the publish folder:
   ```bash
   cp index.html propedge-deploy/index.html
   ```
...
- **Production URL:** https://propedgemasters.netlify.app
- Git commits are still used for version history, but deployment is always via Netlify CLI.
...
PropEdge/
├── CLAUDE.md                    ← This file
├── index.html                   ← Main single-file offline app (also in propedge-deploy/)
├── scraper-v14.js               ← Active scraper (headless, FanDuel-only)
├── run-scraper-daemon.sh        ← Daemon wrapper for launchd cron
├── credentials.json             ← Google Sheets service account key
├── propedge-deploy/             ← Netlify publish folder
└── logs/                        ← Scraper + daemon logs
